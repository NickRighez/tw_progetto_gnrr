	 ...
	 my $arr = $q->param('arrivo');
	  ...
	  if($num!=0) {
	    my %problems=(
	      ...
	      $session->param('problems',\%problems);
	      print $session->header(-location => "http://localhost/cgi-bin/tw_progetto_gnrr/file_sito/PERL/assemblatori/assemblatore_singolo_passaggio.cgi?passaggio=$pass&part=$part&arr=$arr");
	  }
	  else {
	    ...
	    if(data_registration::inserisci_notifica("RichiestaPrenotaz", \%Prenotazione, $conducente)) {
	      print $session->header(-location => "http://localhost/cgi-bin/tw_progetto_gnrr/file_sito/PERL/assemblatori/assemblatore_viaggi.cgi");
	    }
	  }
}
