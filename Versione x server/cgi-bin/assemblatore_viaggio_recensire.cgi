...
if(!defined($session->param('username'))) {
  ...
  print $session->header(-location => "http://tecnologie-web.studenti.math.unipd.it/tecweb/~nrigato/cgi-bin/accedi.cgi"); ####!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
}
...
for(my $i=0; $i<$num; $i++){
	...
	if($conducente eq $dest) {
		...
		<input type=\"radio\" name=\"CompagniaP"."$ind\" id=\"p"."$ind"."Compagnia3\" value=\"3\" tabindex=\"9\" checked=\"checked\"></input><label  for=\"p"."$ind"."Compagnia3\">3</label>
		...
		<input type=\"radio\" name=\"PuntualitaP"."$ind\" id=\"p"."$ind"."Puntualita3\" value=\"3\" tabindex=\"10\" checked=\"checked\"></input><label  for=\"p"."$ind"."Puntualita3\">3</label>
		...
		<textarea rows=\"\" cols=\"\" name=\"commentoP1\" tabindex=\"11\" title=\"Inserisci un commento facoltativo\">Commento facoltativo</textarea>
...
my $file = "../data/HTML_TEMPLATE/TravelShare/feedback.html";
my %hash_keys = (
  USERNAME => $username,
  CONTENUTO => $contenuto,
  PASSAGGIO => $passaggio
